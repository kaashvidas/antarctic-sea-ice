...
| `src/models/seaice_forecast/convlstm.py` | Working architecture. |
| `src/models/seaice_forecast/train.py` | Dataset loader + chronological split + training loop + baseline comparison table all implemented, run end-to-end on synthetic data — awaiting real processed NSIDC history to train on for real. |
...
| `src/integration/pipeline.py` | Full orchestration working end-to-end: real iceberg cluster → real drift ensemble → real bathymetry → cost grid → A*/naive routes → GeoJSON + sea-ice PNGs + manifest. Sea-ice concentration and wind/current forcing are disclosed placeholders (see manifest's `placeholder_inputs`) pending the credential-blocked downloads above. |

...
SIH26059, Ministry of Earth Sciences. Deadline: **2026-09-30**. This document is for anyone (human or AI) picking up work on this repo — it covers what's real and working, what's actually broken or missing, what to fix now vs. later, and a concrete plan for splitting regional expansion across multiple people/machines. It does not replace `README.md` (the project pitch/architecture doc) — read that first for the "what is this and why" framing; this doc is the "what's the actual state and what do I do next" framing. For actually deploying this (Render + Vercel), see `DEPLOYMENT.md`.

Last updated: 2026-09-17. All three planned regions are now resolved: Weddell (original), Prydz Bay (built — sea-ice win, drift validation a real informative failure from grounded icebergs), Ross Sea (built — sea-ice win AND drift validation win), Queen Maud Land (deliberately skipped — no real nearby icebergs as of this date). See section 6 for the full real results on each. See "Live status" below for the Weddell retrain's methodology lesson, still relevant before you compare your own checkpoints.
...
1. **Prydz Bay / Larsemann Hills (near Bharati, India, 69.41°S 76.19°E).** Highest relevance — real, currently-tracked iceberg cluster confirmed right in this sector: D23 (~4° from Bharati), D15A/B/C/D, D34. **Full pipeline run 2026-09-15/16, real results, both good and bad:**
   - **Sea-ice forecast: a real win.** ConvLSTM MAE 0.0524 vs. persistence 0.1931 and climatology 0.0796 (held out, never trained on) — a ~73% reduction vs. persistence. Multi-day autoregressive skill: day 1 MAE 0.0326 → day 7 0.0687 (+110.7%), same real degradation pattern as Weddell, comparable magnitude.
   - **Iceberg drift physics: a real, honest limitation, not a bug.** WDE17 drift validation against real BYU/NIC tracks came out MUCH WORSE than the zero-motion baseline here (40.02 km mean error vs. 0.67 km baseline) — the opposite of Weddell's result. Root cause, confirmed by checking these icebergs' entire real tracked history: several (D15A, D15B, D23) barely move at all in reality (near-zero real baseline error) — the signature of a **grounded iceberg** (stuck on a shoal/bank), which WDE17's free-drift physics has no way to represent; it always predicts continued wind/current-driven motion. Prydz Bay's shallower, more complex coastal bathymetry plausibly grounds bergs more often than the Weddell cluster. **Practical implication: don't present drift predictions for this region with the same confidence as Weddell's until/unless a grounding check is added** (e.g., flag an iceberg whose real recent track shows near-zero net displacement as "likely grounded, drift prediction unreliable" rather than running the free-drift model blind).
   - Several bergs have deep real historical tracks in the BYU database (D15A: 3,392 rows, D15B: 3,682 rows).
2. **Queen Maud Land (near Maitri, India, 70.77°S 11.73°E). SKIPPED 2026-09-16 — real, honest finding, not a data-access failure.** A fresh live USNIC pull found **zero currently-tracked icebergs within ~1,000km of Maitri** — the earlier "D37" placeholder turned out to be wrong even at the provisional stage (D37 is actually at 36.36°E, outside any reasonable Maitri-centered box; the nearest real bergs, D37/C18C, are each ~1,000+ km away). Rather than force a fake cluster or redefine the region ~1,000km off-center, this was deliberately deferred. Revisit if a real berg drifts into range — re-check via a live USNIC pull before resuming, don't reuse this note as evidence either way since iceberg presence changes month to month.
3. **Ross Sea / McMurdo Sound.** Not an Indian station, but a globally significant, heavily-trafficked research hub (McMurdo, Scott Base). **Full pipeline run 2026-09-16, real results:**
   - **Sea-ice forecast: a real win**, consistent with the other two regions. ConvLSTM MAE 0.0497 vs. persistence 0.1790 and climatology 0.0807 (~72% reduction). Multi-day skill: day 1 MAE 0.0302 → day 7 0.0730 (+141.5%), same real degradation magnitude as Weddell/Prydz Bay.
   - **Iceberg drift: a real win too**, unlike Prydz Bay. B22H validated against its real BYU/NIC track: 37.0 km mean error vs. 55.0 km persistence baseline — genuine skill, not grounded. B22A could NOT be validated — not a bug: its entire real tracked history (2002-2026) stays between 107°W and 180°W, meaning it must have drifted into this region's box very recently, after the BYU historical database's last update (April 2026), so there's no overlapping ground-truth window yet. Real data-recency gap, not something to fix.
   - Real cluster is thin (2 icebergs vs. 6 for the other regions) — B22A has deep history (8,645 real rows) once it has time to accumulate in-domain observations; B22H has 287 rows.
...
2. Each person runs, for their region only (`REGION=<name>` before every command — see section 4's usage note):
   ...
   d. **Two separate weather downloads, both needed** (found the hard way bringing up Prydz Bay -- easy to do only one and not notice): `python -m src.data.download_weather --start ... --end ...` for the HISTORICAL archive (training + drift validation), **and also** `python -m src.data.download_weather` with no args for the live forecast-style pull (`wind_openmeteo.csv`/`current_openmeteo.csv`) that `/api/plan_journey` actually reads at request time -- skip the second one and the live app silently falls back to a synthetic placeholder with only a printed warning to notice it by. ~30-60 min combined.
   ...
   f. `python -m src.models.seaice_forecast.evaluate_multiday` (now correctly defaults to the concentration-only file, fixed 2026-09-16) then `python -m src.models.iceberg_drift.validate_drift`. **Read the drift validation result, don't just check it ran** — for Prydz Bay this came back genuinely worse than baseline (see section 6), a real finding about grounded icebergs, not a bug; report results like this honestly rather than assuming validate_drift.py "passing" means the physics model is trustworthy for every region.
   g. Add your own region to `tests/test_journey_report.py`'s `_REGION_TEST_FIXTURES` dict (real start/goal coordinates + a feasible ice class, verified via an actual `plan_journey()` call, not guessed -- a region's real ice cover varies enough that Weddell's or Prydz Bay's coordinates will not work for yours) before your region's tests can run.
3. Commit only region-scoped files (thanks to the refactor, these won't collide with anyone else's) plus any `grid.py`/test-fixture correction from steps 2a/2g.
4. Merge each region branch once its own test suite subset passes (`REGION=<name> pytest tests/ -q`) -- except `test_real_drift_validation_skill`, which is EXPECTED to fail for a region where the tracked icebergs turn out to be grounded rather than drifting; don't treat that specific failure as a merge blocker, just make sure it's understood and documented, not silently ignored.

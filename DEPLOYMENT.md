...
4. Click **Apply** / **Deploy Blueprint**. Render builds the same Docker image three times (once per service — it bakes in all three regions' trimmed data, then each service's `REGION` env var picks which one it actually serves) and starts them. First build takes a few minutes per service (real PyTorch install, not instant).
